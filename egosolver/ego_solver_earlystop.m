function [bestx, bestf, bestc, archive, external_return] = ego_solver_earlystop(funh_obj, num_xvar, lb, ub, initmatrix, funh_con, param, varargin)
...
addParameter(p, 'stopping_count', 50);
...
%------------------------------------------------------
bestf_sofar = []; %for termination condition

for io = 1:size(orgf, 1)
    partf = orgf(1:io, :);
    [fmin_orgf, ~] = min(partf, [], 1);
    bestf_sofar = [bestf_sofar; fmin_orgf];
end
...
% infill process
while ~stopping_condition || n > nmax
    ...
    % place stopping condition
    for io = 1:size(next_f)
        orgx = [orgx; next_x(io, :)];
        orgf = [orgf; next_f(io, :)];

        fmin_orgf = min(orgf, [], 1);
        bestf_sofar = [bestf_sofar; fmin_orgf];
    end
    

    if size(orgf, 1) > nc
        check_bestf = bestf_sofar(end - nc + 1: end, :);
        std_bestf = std(check_bestf); 
        if std_bestf < 1e-3
...
function stopping_condition = check_stopping_condition(nc, orgx, orgf)
% stopping condition is that in n consecutive infills, best solution is
% not changing

last_ncx = orgx(end-nc:end, :);
last_ncf = orgf(end-nc:end, :);
